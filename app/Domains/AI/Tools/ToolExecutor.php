...
final class ToolExecutor
{
     ...
     * @param  array<string, mixed>  $args
     */
    private function executeWrite(?StorePort $storePort, string $accountId, ToolName $tool, array $args): mixed
    {
        if ($storePort === null) {
            throw new StoreAdapterUnavailableException('Store adapter is not available for write tools.');
        }
        ...
        $result = match ($tool) {
            ...
            ToolName::DeleteProduct => $this->deleteProduct($storePort, $args),
            ToolName::UpdateInventory => $this->updateInventory($storePort, $args),
